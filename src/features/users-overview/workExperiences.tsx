...
const WorkExperience = (props: any) => {
    const { totalData, setTotalData, setHasChanged, saving, setSaving, userId } = props;
    ...
    const handleInsert = async () => {
        try {
            InsertedArray.forEach(async id => {
                const insertedDatas = totalData.filter((element: any) => element.id === id);
                await HouseWorkExperienceApi.createHouseWorkExperience(insertedDatas[0])
            });
    ...
    const handleUpdate = async () => {
        try {
            EditedArray.forEach(async id => {
                const editedDatas = totalData.filter((element: any) => element.id === id);
    ...
    const handleSave = () => {
        ...
        handleInsert()
            ...
            .catch((error) => {
                push('Something went wrong when adding work experiences.', { variant: 'error' });
                setSaveState(oldStates => ({
                    ...oldStates,
                    'Inserted': true
                }))
        ...
        handleUpdate()
            ...
            .catch((error) => {
                push('Something went wrong when updating work experiences.', { variant: 'error' });
                setSaveState(oldStates => ({
                    ...oldStates,
                    'Updated': true
                }))
        ...
        handleDeleteing()
            ...
            .catch((error) => {
                push('Something went wrong when deleting work experiences.', { variant: 'error' });
                setSaveState(oldStates => ({
                    ...oldStates,
                    'Deleted': true
                }))
                    ...
                    return (
                        <AccountGrid style={{ position: 'relative', marginBottom: '20px' }}>
                            <Input
                                ...
                                onValue={(jobTitle) => handleChange("jobTitle", jobTitle, experience.id)}
                            />
                            <Input
                                type="select"
                                label="Company"
                                ...
                                onValue={(company) => handleChange("company", company ? company.value : company, experience.id)}
                            />
                            <Input
                                type="select"
                                label="Location"
                                ...
                                onValue={(location) => handleChange("location", location ? location.value : location, experience.id)}
                            />
                            <Stack direction="horizontal" style={{ position: 'relative' }}>
                                ...
                                <Input
                                    ...
                                    label="To"
                                    placeholder="Please Select"
                                    value={experience.stillWorkHere ? new Date().toISOString() : experience.to}
                                ...
                                <Checkbox
                                    ...
                                    onValue={(stillWorkHere) => handleChange("stillWorkHere", stillWorkHere, experience.id)}
                                />
                            </Stack>
                            <Input
                                type="text"
                                ...
                                onValue={(roleDescription) => handleChange("roleDescription", roleDescription, experience.id)}
                            />
                            <Stack
                                style={{
                                    position: 'absolute',
                            ...
                            >
                                <Stack style={{ cursor: 'pointer' }} onClick={() => handleDelete(experience.id)}>
